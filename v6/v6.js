...
function startEngine() {
  ...
  var basePos = new Float32Array(COUNT*3), colArr = new Float32Array(COUNT*3);
  var inkC = new THREE.Color(COL.ink);
  (function () { var k = 0;
  ...
  function applyCamera(){
    ...
    if (gradeEl){
      ...
      gradeEl.style.background = "radial-gradient(58% 50% at 62% 32%, rgba("+rgb+",.5), rgba("+rgb+",0) 70%)";
      gradeEl.style.opacity = (inf*(isWorld?0.42:0.18)).toFixed(3);
    }
  ...
  function frame(){
    ...
    for (var ci=0;ci<COUNT*3;ci+=3){ colA.array[ci]+=(inkC.r-colA.array[ci])*0.06; colA.array[ci+1]+=(inkC.g-colA.array[ci+1])*0.06; colA.array[ci+2]+=(inkC.b-colA.array[ci+2])*0.06; }
    ...
    if (SN){ var len = warp*warp*32;
      ...
      dots.material.opacity = 0.4 - warp*0.22;
    }
...
function initLea(){
  ...
  if (orb && panelLea){
    ["Show me the €200 grant pathway →","Will I pass next week's inspection?","Which pack fits my trade?","Talk to a human"].forEach(function (cq){
      var b=document.createElement("button"); b.type="button"; b.textContent=cq; b.addEventListener("click", function (){ addMsg(cq,"user"); botReply(cq); }); chipsBox.appendChild(b); });
    var greeted=false;
