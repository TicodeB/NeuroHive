...
### 🌍 v5 FULL PRODUCT SPEC (Samuel, 11/06 late — THE blueprint, supersedes earlier scopes)
**Worlds (8):** Atrium → Shops on the Street → Trades People → Manufacturing → Farmers (land & livestock) → Schools & Education → Doctors & Dentists → **the €200 Grant world**. "Don't focus only on retail." All wired into BEATS + portals + compass + per-world fog/accent (11/06).
**Per microsite, the conversion machine:** pain-point story → witty Leanta solution → CTA: automation + site visit with TIME+MONEY-SAVING value prop sold as a product → **3D animated avatar** (helpful, funny, SPEAKS with lovely voice; sells/converts or routes to before→after changeover videos) → upsell preparedness magnets + **call scheduler** + yes/no grant question → industry-specific common-services upsell.
**Grant world requirements:** final LEO application form ready to send · **Twilio** text-us flow (needs Samuel's Twilio creds) · "tell us your pain point, we come back with solution" capture · shortcut to Excel/digital shop ("everything now, no automation, first step").
...
### ⏭ NEXT SESSION
-1. **BINDING — 3D ENGINE RETURNS (Samuel, 10/06 night): "We need 3D engine — I just didn't like your asset of exploding papers."** The v4.3 three.js removal overcorrected: the objection was the paper-storm CONTENT, not WebGL. NORTHSTAR commandment 2 amended: procedural 3D is allowed when it's the brand signature, never random decoration. NEXT BUILD: port the shipped Canvas2D mesh field (symmetric dot lattice, wave, Z-dolly on scroll, cursor swarm + ink→emerald hover) to **three.js instanced points** (10–20k dots, fog, depth-scaled size) as the site-wide signature layer; make the **mind map truly 3D** (nodes as floating glass orbs in space, emerald light-lines, slow camera orbit + scroll unfold — the CSS tilt shipped today is the placeholder); hero beautification pass rides on both. Canvas2D mesh stays as the no-WebGL fallback.
