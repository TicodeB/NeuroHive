...
- **DB now has:** `standards` (21), `compliance_assets` (19), view `v_audit_packs` (115 rows), `products` (**20 rows** — 12 Phase-7 + 8 Phase-11; `audience` + `standard_ids` populated). **Re-seed ORDER MATTERS:** `python3 scripts/seed_products.py` (12 base — wipes `products`) **THEN** `python3 scripts/seed_compliance.py` (re-enriches the 12 + adds the 8 audit/compliance rows P13–P20 = 20). The 8 audit products live in `seed_compliance.py` (`AUDIT_PRODUCTS`), not `seed_products.py`, because they bundle `compliance_assets`.
